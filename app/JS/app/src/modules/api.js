...
const api = {
  ...
  getaevents({id=null, options={}}={}){
    // defaults
    options.f = options.f || 'geojson';

    // make request
    return request(`/aevents${id ? '/' + id: ''}`, options);
  },

  getaembassadorFromaevent(aeventId, options={}){
    if (!aeventId){
      return Promise.resolve([]);
    }
    return request(`/aevents/${aeventId}/aembassador`, options);
  },

  getaembassador(id, options={}){
    return request(`/aembassador/${id}`, options);
  },

  async getStyles(options, asOptions=true){
    const resp = await request('/aembassador/styles', options);
    if (asOptions){
  ...
  getaembassadorPhotos(aembassadorId, options={}){
    return request(`/aembassador/${aembassadorId}/photos`, options);
  },

  getPhotoUrl(photo_id, cacheBust=true){
    // need to full url since this will likely be used in an <img> tag
    return `${axios.defaults.baseURL}/aembassador/photos/${photo_id}/download${cacheBust ? '?cb=' + new Date().getTime(): ''}`;
  },
  ...
  async exportData({table='aevents', format='csv'}={}){
    try {
  ...
  async uploadaembassadorPhoto(aembassador_id, file, photoId=null){
    ...
    formData.append('aembassador_id', aembassador_id);

    // return response
    const url = `/data/aembassador_photos/${parseInt(photoId) > 0 ? photoId + '/update': 'create'}`;
    console.log(url)
