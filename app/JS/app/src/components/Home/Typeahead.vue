    ...
    <b-list-group v-show="hasItems" class="mt-1 search-results">
      <b-list-group-item v-for="(item, $item) in items" :key="item.id"
        ...
        @mousemove="setActive($item)">
          <p class="event-name hit-result mb-2">{{ item.name }}</p>
...
<script>
  ...
  export default {
    ...
    computed: {
        src(){
          return `${this.$root.config.api_base}/aevent`
    ...
    methods: {
      onHit (item) {
        ...
        EventBus.$emit('event-search-result', {
          type: 'Feature',
          properties: item,
          geometry: {
            coordinates: [
...
<style scoped>
  ...
  .typeahead-input:focus {
    border-color: #f7296E;
    outline: 0;
    box-shadow: inset 0 1px 1px rgba(0,0,0,.075),0 0 8px #f7296E;
  }
