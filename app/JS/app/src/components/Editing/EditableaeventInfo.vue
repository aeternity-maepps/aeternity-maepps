<template>
  <b-card bg-variant="light" class="editable-aembassador" body-class="card-block">

    <!--  SPINNER FOR LOADING -->
    <span style="font-size: 3.5rem;" class="centered" v-if="state ==='loading'">
      <spinner :text="'loading aembassador info...'" :visible="true"/>
    </span>
    ...
    <b-container class="mx-auto" v-else>
      ...
      <b-row class="mt-4">
        <b-col md="10" sm="12" align-h="center">
          <b-form-group label="Name:"
                        ...
                        :label-cols="2">
            <b-form-input v-model="aembassador.name" style="font-weight: bold;" />
          </b-form-group>
        </b-col>
      ...
      <b-row class="mt-2" align-h="center">
        <b-col :sm="prop.cols * 2" :md="prop.cols" v-for="prop in aembassador_props" :key="prop.field">
          <b-form-group :label="prop.label + ':'" label-text-align="left">
            <b-form-input :type="prop.type" v-model="aembassador[prop.field]" />
          </b-form-group>
        </b-col>
      </b-row>
      ...
      <b-row class="mt-2">
        <b-col md="10" sm="12" align-h="center">
          <b-form-group label="Style:"
                        ...
                        label-text-align="right"
                        :label-cols="2">
            <b-form-select :options="aembassadorStyles" v-model="aembassador.style" />
      ...
      <b-row class="mt-2">
        <b-col md="10" sm="12" align-h="center">
          <b-form-group label="Description:"
                        ...
                        label-text-align="right"
                        :label-cols="2">
            <b-form-textarea v-model="aembassador.description" :rows="6" />
          </b-form-group>
        </b-col>
...
<script>
  ...
  export default {
    ...
    async mounted(){
      hook.eb = this;
      console.log('mounted editable aembassador: ', this.$route.params);
      const styles = await api.getStyles();
      this.aembassadorStyles.length = 0;
      this.aembassadorStyles.push(...styles);
    },

    // we want to make sure to intercept this to force the router to update
    // the current aembassador
    beforeRouteEnter(to, from, next){
      console.log('BEFORE aembassador ROUTE UPDATE: ', to, from, next);
      next(async (vm) => {
        await vm.update(to.params.aembassador_id);
        console.log('updated aembassador and calling next: ', vm.aembassador);
        next();
        window.scrollTo(0,0);
      });
    ...
    beforeRouteLeave (to, from, next){
      ...
      if (JSON.stringify(this.aembassador) != JSON.stringify(this.copy)) {
        swal({
          type: 'warning',
          ...
          if (choice.value) {
            ...
            console.log('SAVE aembassador HERE!');
            this.saveChanges();
          }
    ...
    methods: {
      async update(id){
        this.state = 'loading';
        if (!id){
          id = this.$route.params.aembassador_id;
        }
        this.aembassador = await api.getaembassador(id);
        this.copy = Object.assign({}, this.aembassador);
        this.photoInfos.length = 0;
        const photoInfos = await api.getaembassadorPhotos(this.aembassador.id);
        ...
        console.log('loaded aembassador: ', this.aembassador);
        this.state = 'loaded';
        return this.aembassador;
      },

      emitaembassadorChange(){
        EventBus.$emit('aembassadors-changed', {
          aevent_id: this.aembassador.aevent_id,
          aembassador_id: this.aembassador.id,
          type: 'update'
        });
      },
      ...
      async saveChanges(){
       ...
       this.state = 'saving';
        try {
          const resp = await api.updateItem('aembassadors', this.aembassador);

          // make sure to update copy so router guard isn't thrown
          this.copy = Object.assign({}, this.aembassador);
          this.emitaembassadorChange();
      ...
      async photoHandler(files){
        ...
        try{
          const resp = await api.uploadaembassadorPhoto(this.aembassador.id, photo, photoId);
