...
# safe imports
__all__ = ('aembassador', 'aevent', 'aembassadorPhotos', 'User', 'Category', 'Style', 'engine', 'Base', 'session')
...
aevent_str = 'sqlite:///{}/aembassador.db?check_same_thread=False'.format(db_path) 
...
class aembassadorPhotos(Base):
    __tablename__ = 'aembassador_photos'
    id = Column(Integer, primary_key=True, autoincrement=True)
    aembassador_id = Column(Integer, ForeignKey('aembassadors.id'))
...
class aembassador(Base):
    __tablename__ = 'aembassadors'
    id = Column(Integer, primary_key=True, autoincrement=True)
    aevent_id = Column(Integer, ForeignKey('aevents.id'))
    ...
    photos = relationship(aembassadorPhotos, cascade="all, delete-orphan")
    created_by = Column(Integer, ForeignKey('users.id'))
    aevent = relationship('aevent', back_populates='aembassadors')
    user = relationship('User', back_populates='submitted_aembassadors')
...
class aevent(Base):
    ...
    brew_type = Column(String(50), default='aevent')
    website = Column(String(255), default=None)
    x = Column(Float)
    y = Column(Float)
    created_by = Column(Integer, ForeignKey('users.id'))
    user = relationship('User', back_populates='submitted_aevents')
    aembassadors = relationship('aembassador', back_populates='aevent', cascade="all, delete-orphan")
...
class User(Base, UserMixin):
    ...
    submitted_aevents = relationship('aevent')
    submitted_aembassadors = relationship('aembassador')
...
engine = create_engine(aevent_str)
